(function ($) {
    ...
    $( "#slider-range" ).slider({
        ...
        slide: function( event, ui ) {
            $("#amount").val("تومان" + ui.values[0].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,') + " - تومان " + ui.values[1].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,'));
            $("#amountMinVal").val(ui.values[0]);
            $("#amountMaxVal").val(ui.values[1]);
            FilterProducts();
       }
    });
    $("#amount").val("تومان" + $("#slider-range").slider("values", 0).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,') +
        " - تومان" + $("#slider-range").slider("values", 1).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,') );
